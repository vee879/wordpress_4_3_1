...
if (!function_exists('shortcode_jw_twitter')) {
    ...
    function shortcode_jw_twitter($atts, $content) {
        ...
        if (is_array($jw_twitter_tweets)) {
            ...
            $output.='</ul>';
            //$output.='<div class="twitter-follow">'  . (jw_option('jw_car_follow') ? jw_option('jw_car_follow') : __('Follow Us', 'joinwebs')) . ' - <a target="_blank" href="http://twitter.com/' . $atts['username'] . '">@' . $atts['username'] . '</a></div>';
            $output.='</div>';
            return $output;
        } else {
            return $jw_twitter_tweets;
